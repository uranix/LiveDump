...
{
  bx_cpu. gen_reg[i->b1 & 0x07].word.rx = i->Iw;
}
...
bx_cpu_c::XCHG_RXAX(BxInstruction_t *i)
{
  ...
  temp16 = AX;
  AX = bx_cpu. gen_reg[i->b1 & 0x07].word.rx;
  bx_cpu. gen_reg[i->b1 & 0x07].word.rx = temp16;
}
...
bx_cpu_c::MOV_EwSw(BxInstruction_t *i)
{
  ...
  seg_reg = bx_cpu. sregs[i->nnn].selector.value;
...
bx_cpu_c::MOV_SwEw(BxInstruction_t *i)
{
  ...
  load_seg_reg(&bx_cpu. sregs[i->nnn], op2_16);
