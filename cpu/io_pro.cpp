...
bx_cpu_c::allow_io(Bit16u addr, unsigned len)
{
  ...
  if (bx_cpu. tr.cache.u.tss386.limit_scaled < 103) {
    BX_PANIC(("allow_io(): TR.limit < 103"));
    }

  access_linear(bx_cpu. tr.cache.u.tss386.base + 102, 2, 0, BX_READ,
                         &io_base);
  if (io_base <= 103) {
BX_INFO(("PE is %u", bx_cpu. cr0.pe));
BX_INFO(("VM is %u", bx_cpu. eflags.vm));
...
BX_INFO(("len is %u", len));
  ...
  if (io_base > bx_cpu. tr.cache.u.tss386.limit_scaled) {
    BX_INFO(("allow_io(): CPL > IOPL: no IO bitmap defined #GP(0)"));
    return(0);
    }

  access_linear(bx_cpu. tr.cache.u.tss386.base + io_base + addr/8,
