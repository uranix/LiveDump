...
#endif
          if (BX_NULL_SEG_REG(instruction->seg))
            instruction->seg = bx_cpu. sreg_mod01_rm32[rm];
get_8bit_displ:
          if (ilen < remain) {
...
#endif
        if (BX_NULL_SEG_REG(instruction->seg))
          instruction->seg = bx_cpu. sreg_mod10_rm32[rm];
get_32bit_displ:
        if ((ilen+3) < remain) {
          Bit32u imm32u;
          imm32u = *iptr++;
...
#endif
          if (BX_NULL_SEG_REG(instruction->seg))
            instruction->seg = bx_cpu. sreg_mod0_base32[base];
...
#endif
          if (BX_NULL_SEG_REG(instruction->seg))
            instruction->seg = bx_cpu. sreg_mod1or2_base32[base];
          goto get_8bit_displ;
          }
...
#endif
        if (BX_NULL_SEG_REG(instruction->seg))
          instruction->seg = bx_cpu. sreg_mod01_rm16[rm];
#if BX_DYNAMIC_TRANSLATION
...
#endif
        if (BX_NULL_SEG_REG(instruction->seg))
          instruction->seg = bx_cpu. sreg_mod10_rm16[rm];
#if BX_DYNAMIC_TRANSLATION
        instruction->DTMemRegsUsed = BxMemRegsUsed16[rm];
#endif
        if ((ilen+1) < remain) {
...
#endif
      if (BX_NULL_SEG_REG(instruction->seg))
        instruction->seg = bx_cpu. sreg_mod00_rm16[rm];
      if (rm == 0x06) {
        if ((ilen+1) < remain) {
          Bit16u displ16u;
          displ16u = *iptr++;
