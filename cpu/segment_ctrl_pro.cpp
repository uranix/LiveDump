...
bx_cpu_c::load_seg_reg(bx_segment_reg_t *seg, Bit16u new_value)
{
  ...
  if (v8086_mode()) {
    ...
    if (seg == &bx_cpu. sregs[BX_SREG_CS])
...
#if BX_CPU_LEVEL >= 2
  if (protected_mode()) {
    if (seg == &bx_cpu. sregs[BX_SREG_SS]) {
      Bit16u index;
      Bit8u ti;
      Bit8u rpl;
      ...
      if (ti == 0) { /* GDT */
        if ((index*8 + 7) > bx_cpu. gdtr.limit) {
          BX_PANIC(("load_seg_reg: GDT: %s: index(%04x*8+7) > limit(%06x)",
            bx_cpu. strseg(seg), (unsigned) index, (unsigned) bx_cpu. gdtr.limit));
          exception(BX_GP_EXCEPTION, new_value & 0xfffc, 0);
          return;
          }
        access_linear(bx_cpu. gdtr.base + index*8,     4, 0,
          BX_READ, &dword1);
        access_linear(bx_cpu. gdtr.base + index*8 + 4, 4, 0,
          BX_READ, &dword2);
        }
      else { /* LDT */
        ...
        access_linear(bx_cpu. ldtr.cache.u.ldt.base + index*8,     4, 0,
          BX_READ, &dword1);
        access_linear(bx_cpu. ldtr.cache.u.ldt.base + index*8 + 4, 4, 0,
          BX_READ, &dword2);
        }
      ...
      bx_cpu. sregs[BX_SEG_REG_SS].selector.value        = new_value;
      bx_cpu. sregs[BX_SEG_REG_SS].selector.index        = index;
      bx_cpu. sregs[BX_SEG_REG_SS].selector.ti           = ti;
      bx_cpu. sregs[BX_SEG_REG_SS].selector.rpl          = rpl;
      bx_cpu. sregs[BX_SEG_REG_SS].cache = descriptor;
    ...
    else if ( (seg==&bx_cpu. sregs[BX_SREG_DS]) ||
              (seg==&bx_cpu. sregs[BX_SREG_ES])
#if BX_CPU_LEVEL >= 3
           || (seg==&bx_cpu. sregs[BX_SREG_FS]) ||
              (seg==&bx_cpu. sregs[BX_SREG_GS])
#endif
            ) {
      Bit16u index;
      Bit8u ti;
  ...
  /* ??? */
  if (seg == &bx_cpu. sregs[BX_SREG_CS]) {
    bx_cpu. sregs[BX_SEG_REG_CS].selector.value = new_value;
    bx_cpu. sregs[BX_SEG_REG_CS].cache.valid = 1;
    bx_cpu. sregs[BX_SEG_REG_CS].cache.p = 1;
    bx_cpu. sregs[BX_SEG_REG_CS].cache.dpl = 0;
    bx_cpu. sregs[BX_SEG_REG_CS].cache.segment = 1; /* regular segment */
    bx_cpu. sregs[BX_SEG_REG_CS].cache.u.segment.executable = 1; /* code segment */
    bx_cpu. sregs[BX_SEG_REG_CS].cache.u.segment.c_ed = 0; /* expand up */
    bx_cpu. sregs[BX_SEG_REG_CS].cache.u.segment.r_w = 1; /* writeable */
    bx_cpu. sregs[BX_SEG_REG_CS].cache.u.segment.a = 1; /* accessed */
    bx_cpu. sregs[BX_SEG_REG_CS].cache.u.segment.base = new_value << 4;
    ...
    bx_cpu. sregs[BX_SEG_REG_CS].cache.u.segment.d_b   = 0; /* default 16bit size */
...
bx_cpu_c::load_cs(bx_selector_t *selector, bx_descriptor_t *descriptor,
           Bit8u cpl)
{
  bx_cpu. sregs[BX_SEG_REG_CS].selector     = *selector;
  bx_cpu. sregs[BX_SEG_REG_CS].cache        = *descriptor;

  /* caller may request different CPL then in selector */
  bx_cpu. sregs[BX_SEG_REG_CS].selector.rpl = cpl;
  bx_cpu. sregs[BX_SEG_REG_CS].cache.valid = 1; /* ??? */
  // (BW) Added cpl to the selector value.
  bx_cpu. sregs[BX_SEG_REG_CS].selector.value =
    (0xfffc & bx_cpu. sregs[BX_SEG_REG_CS].selector.value) | cpl;
}

  void
bx_cpu_c::load_ss(bx_selector_t *selector, bx_descriptor_t *descriptor, Bit8u cpl)
{
  bx_cpu. sregs[BX_SEG_REG_SS].selector = *selector;
  bx_cpu. sregs[BX_SEG_REG_SS].cache = *descriptor;
  bx_cpu. sregs[BX_SEG_REG_SS].selector.rpl = cpl;

  if ( (bx_cpu. sregs[BX_SEG_REG_SS].selector.value & 0xfffc) == 0 )
    BX_PANIC(("load_ss(): null selector passed"));

  if ( !bx_cpu. sregs[BX_SEG_REG_SS].cache.valid ) {
...
BX_INFO(("gdtr.limit = %u", (unsigned) bx_cpu. gdtr.limit));
BX_INFO(("fetch_raw_descriptor: GDT: index > limit"));
BX_INFO(("-----------------------------------"));
      exception(exception_no, selector->value & 0xfffc, 0);
      return;
      }
    access_linear(bx_cpu. gdtr.base + selector->index*8,     4, 0,
      BX_READ, dword1);
    access_linear(bx_cpu. gdtr.base + selector->index*8 + 4, 4, 0,
      BX_READ, dword2);
    }
  else { /* LDT */
    if (bx_cpu. ldtr.cache.valid==0) {
    ...
    access_linear(bx_cpu. ldtr.cache.u.ldt.base + selector->index*8,     4, 0,
      BX_READ, dword1);
    access_linear(bx_cpu. ldtr.cache.u.ldt.base + selector->index*8 + 4, 4, 0,
      BX_READ, dword2);
    }
}
#endif
