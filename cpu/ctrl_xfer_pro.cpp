  ...
  if ( descriptor.segment ) {
    ...
    switch ( descriptor.type ) {
      ...
      case  9: // 386 available TSS
        ...
        if (EIP > bx_cpu. sregs[BX_SEG_REG_CS].cache.u.segment.limit_scaled) {
...
//BX_INFO(("jump_pro: task gate"));
        ...
        if (bx_cpu. sregs[BX_SEG_REG_CS].cache.u.segment.d_b)
          temp_eIP = EIP;
        else
          temp_eIP =  IP;
        if (temp_eIP > bx_cpu. sregs[BX_SEG_REG_CS].cache.u.segment.limit_scaled) {
...
bx_cpu_c::call_protected(BxInstruction_t *i, Bit16u cs_raw, Bit32u disp32)
{
  ...
  if (cs_descriptor.segment) { // normal segment
    ...
    if (i->os_32) {
      if ( !can_push(&bx_cpu. sregs[BX_SEG_REG_SS].cache, temp_ESP, 8) ) {
      ...
      push_32((Bit32u) bx_cpu. sregs[BX_SEG_REG_CS].selector.value);
      push_32(EIP);
      }
    else { // 16bit opsize
...
/* ??? use dpl or rpl ??? */
          ...
          return_SS = bx_cpu. sregs[BX_SEG_REG_SS].selector.value;
          if (bx_cpu. sregs[BX_SEG_REG_SS].cache.u.segment.d_b)
            return_ESP = ESP;
          else
            return_ESP =  SP;
          return_ss_base = bx_cpu. sregs[BX_SEG_REG_SS].cache.u.segment.base;

          // save return CS:eIP to be pushed on new stack
          return_CS = bx_cpu. sregs[BX_SEG_REG_CS].selector.value;
          if ( cs_descriptor.u.segment.d_b )
            return_EIP = EIP;
          else
            return_EIP =  IP;
          ...
          if (gate_descriptor.type==4) {
            for (i=param_count; i>0; i--) {
              ...
              //access_linear(bx_cpu. sregs[BX_SEG_REG_SS].cache.u.segment.base + temp_ESP + i*2,
            ...
            for (i=param_count; i>0; i--) {
              ...
              //access_linear(bx_cpu. sregs[BX_SEG_REG_SS].cache.u.segment.base + temp_ESP + i*4,
...
bx_cpu_c::return_protected(BxInstruction_t *i, Bit16u pop_bytes)
{
  ...
  if (bx_cpu. sregs[BX_SEG_REG_SS].cache.u.segment.d_b) temp_ESP = ESP;
  else temp_ESP = SP;

  // return selector RPL must be >= CPL, else #GP(return selector)
  access_linear(bx_cpu. sregs[BX_SEG_REG_SS].cache.u.segment.base + temp_ESP +
...
//  bx_cpu. prev_eip));
    ...
    if (i->os_32) {
      access_linear(bx_cpu. sregs[BX_SEG_REG_SS].cache.u.segment.base + temp_ESP + 12 + pop_bytes,
        2, 0, BX_READ, &raw_ss_selector);
      access_linear(bx_cpu. sregs[BX_SEG_REG_SS].cache.u.segment.base + temp_ESP + 8 + pop_bytes,
      ...
      access_linear(bx_cpu. sregs[BX_SEG_REG_SS].cache.u.segment.base + temp_ESP + 6 + pop_bytes,
        2, 0, BX_READ, &raw_ss_selector);
      access_linear(bx_cpu. sregs[BX_SEG_REG_SS].cache.u.segment.base + temp_ESP + 4 + pop_bytes,
...
bx_cpu_c::iret_protected(BxInstruction_t *i)
{
  ...
  if (bx_cpu. eflags.nt) { /* NT = 1: RETURN FROM NESTED TASK */
    ...
    if (bx_cpu. tr.cache.type == 1)
      base32 = bx_cpu. tr.cache.u.tss286.base;
    else if (bx_cpu. tr.cache.type == 9)
      base32 = bx_cpu. tr.cache.u.tss386.base;
    else {
      BX_PANIC(("IRET: TR not valid"));
    ...
    access_linear(bx_cpu. sregs[BX_SEG_REG_SS].cache.u.segment.base + temp_ESP + cs_offset,
      2, CPL==3, BX_READ, &raw_cs_selector);

    if (i->os_32) {
      access_linear(bx_cpu. sregs[BX_SEG_REG_SS].cache.u.segment.base + temp_ESP + 0,
        4, CPL==3, BX_READ, &new_eip);
      access_linear(bx_cpu. sregs[BX_SEG_REG_SS].cache.u.segment.base + temp_ESP + 8,
      ...
      if (new_eflags & 0x00020000) {
        ...
        bx_cpu. stack_return_to_v86(new_eip, raw_cs_selector, new_eflags);
        return;
        }
      }
    else {
      ...
      access_linear(bx_cpu. sregs[BX_SEG_REG_SS].cache.u.segment.base + temp_ESP + 4,
    ...
    if (cs_selector.rpl == CPL) { /* INTERRUPT RETURN TO SAME LEVEL */
      ...
      /* examine return SS selector and associated descriptor */
      access_linear(bx_cpu. sregs[BX_SEG_REG_SS].cache.u.segment.base + temp_ESP + ss_offset,
        2, 0, BX_READ, &raw_ss_selector);
      ...
      if (i->os_32) {
        ...
        access_linear(bx_cpu. sregs[BX_SEG_REG_SS].cache.u.segment.base + temp_ESP + 12,
          4, 0, BX_READ, &new_esp);
        }
      else {
        access_linear(bx_cpu. sregs[BX_SEG_REG_SS].cache.u.segment.base + temp_ESP + 0,
        ...
        access_linear(bx_cpu. sregs[BX_SEG_REG_SS].cache.u.segment.base + temp_ESP + 6,
          2, 0, BX_READ, &new_sp);
        new_eip = new_ip;
        new_esp = new_sp;
...
{
  if ( bx_cpu. sregs[BX_SEG_REG_ES].cache.dpl<CPL ) {
    bx_cpu. sregs[BX_SEG_REG_ES].cache.valid = 0;
    bx_cpu. sregs[BX_SEG_REG_ES].selector.value = 0;
    }
  if ( bx_cpu. sregs[BX_SEG_REG_DS].cache.dpl<CPL ) {
    bx_cpu. sregs[BX_SEG_REG_DS].cache.valid = 0;
    bx_cpu. sregs[BX_SEG_REG_DS].selector.value = 0;
    }
  if ( bx_cpu. sregs[BX_SEG_REG_FS].cache.dpl<CPL ) {
    bx_cpu. sregs[BX_SEG_REG_FS].cache.valid = 0;
    bx_cpu. sregs[BX_SEG_REG_FS].selector.value = 0;
    }
  if ( bx_cpu. sregs[BX_SEG_REG_GS].cache.dpl<CPL ) {
    bx_cpu. sregs[BX_SEG_REG_GS].cache.valid = 0;
    bx_cpu. sregs[BX_SEG_REG_GS].selector.value = 0;
    }
}
#endif
