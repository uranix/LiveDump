...
//  unsigned prev_errno;
  ...
  BX_DEBUG(("interrupt(): vector = %u, INT = %u, EXT = %u",
      (unsigned) vector, (unsigned) is_INT, (unsigned) bx_cpu. EXT));

bx_cpu. save_cs  = bx_cpu. sregs[BX_SEG_REG_CS];
bx_cpu. save_ss  = bx_cpu. sregs[BX_SEG_REG_SS];
bx_cpu. save_eip = EIP;
bx_cpu. save_esp = ESP;

//  prev_errno = bx_cpu. errorno;
  ...
  if(!real_mode()) {
    ...
    if ( (vector*8 + 7) > bx_cpu. idtr.limit) {
      BX_DEBUG(("IDT.limit = %04x", (unsigned) bx_cpu. idtr.limit));
      BX_DEBUG(("IDT.base  = %06x", (unsigned) bx_cpu. idtr.base));
    ...
    access_linear(bx_cpu. idtr.base + vector*8,     4, 0,
      BX_READ, &dword1);
    access_linear(bx_cpu. idtr.base + vector*8 + 4, 4, 0,
      BX_READ, &dword2);
...
// new stack must have room for 10/12 bytes, else #SS(seg selector)
          ...
          old_ESP = ESP;
          old_SS  = bx_cpu. sregs[BX_SEG_REG_SS].selector.value;
          old_EIP = EIP;
          old_CS  = bx_cpu. sregs[BX_SEG_REG_CS].selector.value;
        ...
        if ( cs_descriptor.u.segment.c_ed==1 || cs_descriptor.dpl==CPL ) {
          ...
          if ( !can_push(&bx_cpu. sregs[BX_SEG_REG_SS].cache,
                         temp_ESP, bytes) ) {
    ...
    cs_selector = bx_cpu. sregs[BX_SEG_REG_CS].selector.value;
    push_16(cs_selector);
    ip = bx_cpu. eip;
    push_16(ip);

    access_linear(bx_cpu. idtr.base + 4 * vector,     2, 0, BX_READ, &ip);
    IP = ip;
    access_linear(bx_cpu. idtr.base + 4 * vector + 2, 2, 0, BX_READ, &cs_selector);
    load_seg_reg(&bx_cpu. sregs[BX_SEG_REG_CS], cs_selector);
  ...
  // error_code: if exception generates and error, push this error code
{
  ...
  BX_PANIC(("exception triggered (code = %x) eip = 0x%.8x", error_code, EIP));
...
//BX_DEBUG(( "::exception(%u)", vector ));
  ...
  if (bx_cpu. errorno) {
    bx_cpu. sregs[BX_SEG_REG_CS]  = bx_cpu. save_cs;
    bx_cpu. sregs[BX_SEG_REG_SS]  = bx_cpu. save_ss;
    EIP = bx_cpu. save_eip;
    ESP = bx_cpu. save_esp;
    }
  ...
  if ( (bx_cpu. errorno==2) && (bx_cpu. curr_exception[0]==BX_ET_DOUBLE_FAULT) ) {
   ...
   */
  if (bx_cpu. errorno >= 2) {
    if (is_exception_OK[bx_cpu. curr_exception[0]][exception_type])
      bx_cpu. curr_exception[1] = exception_type;
    else {
      bx_cpu. curr_exception[1] = BX_ET_DOUBLE_FAULT;
      vector = 8;
    }
  }
  else {
    bx_cpu. curr_exception[0] = exception_type;
  }
  ...
  if (!real_mode()) {
    prev_errno = bx_cpu. errorno;
    bx_cpu. interrupt(vector, 0, push_error, error_code);
//    if (bx_cpu. errorno > prev_errno) {
...
//        }
//      }
    bx_cpu. errorno = 0; // error resolved
    longjmp(bx_cpu. jmp_buf_env, 1); // go back to main decode loop
    }
  else // real mode
#endif
    {
    // not INT, no error code pushed
    bx_cpu. interrupt(vector, 0, 0, 0);
    bx_cpu. errorno = 0; // error resolved
    longjmp(bx_cpu. jmp_buf_env, 1); // go back to main decode loop
    }
}
...
{
  if (seg == &bx_cpu. sregs[BX_SEG_REG_SS])
