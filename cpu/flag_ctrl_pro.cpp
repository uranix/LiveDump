...
{
  bx_cpu. set_CF(flags & 0x01);
  bx_cpu. set_PF((flags >> 2) & 0x01);
  bx_cpu. set_AF((flags >> 4) & 0x01);
  bx_cpu. set_ZF((flags >> 6) & 0x01);
  bx_cpu. set_SF((flags >> 7) & 0x01);

#if 0
// +++
if (bx_cpu. eflags.tf==0 && (flags&0x0100))
  BX_DEBUG(( "TF 0->1" ));
else if (bx_cpu. eflags.tf && !(flags&0x0100))
  BX_DEBUG(( "TF 1->0" ));
else if (bx_cpu. eflags.tf && (flags&0x0100))
  BX_DEBUG(( "TF 1->1" ));
#endif
...
{
  bx_cpu. set_CF(eflags_raw & 0x01);
  bx_cpu. set_PF((eflags_raw >> 2) & 0x01);
  bx_cpu. set_AF((eflags_raw >> 4) & 0x01);
  bx_cpu. set_ZF((eflags_raw >> 6) & 0x01);
  bx_cpu. set_SF((eflags_raw >> 7) & 0x01);

#if 0
// +++
if (bx_cpu. eflags.tf==0 && (eflags_raw&0x0100))
  BX_DEBUG(( "TF 0->1" ));
else if (bx_cpu. eflags.tf && !(eflags_raw&0x0100))
  BX_DEBUG(( "TF 1->0" ));
else if (bx_cpu. eflags.tf && (eflags_raw&0x0100))
  BX_DEBUG(( "TF 1->1" ));
#endif

  bx_cpu. eflags.tf = (eflags_raw >> 8) & 0x01;
  if (bx_cpu. eflags.tf) {
    bx_cpu. async_event = 1;
    }

  if (change_IF)
    bx_cpu. eflags.if_ = (eflags_raw >> 9) & 0x01;

  bx_cpu. eflags.df = (eflags_raw >> 10) & 0x01;
  bx_cpu. set_OF((eflags_raw >> 11) & 0x01);

  if (change_IOPL)
    bx_cpu. eflags.iopl = (eflags_raw >> 12) & 0x03;
  bx_cpu. eflags.nt = (eflags_raw >> 14) & 0x01;

  if (change_VM) {
    bx_cpu. eflags.vm = (eflags_raw >> 17) & 0x01;
  ...
  if (change_RF) {
    bx_cpu. eflags.rf = (eflags_raw >> 16) & 0x01;
    }

#if BX_CPU_LEVEL >= 4
  bx_cpu. eflags.ac = (eflags_raw >> 18) & 0x01;
  bx_cpu. eflags.id = (eflags_raw >> 21) & 0x01;
#endif
