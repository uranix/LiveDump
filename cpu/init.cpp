...
bx_cpu_c::reset(unsigned source)
{
  ...
  bx_cpu. lf_flags_status = 0x000000;
  bx_cpu. lf_pf = 0;
   ...
   */
  bx_cpu. sregs[BX_SEG_REG_CS].selector.value =     0xf000;
#if BX_CPU_LEVEL >= 2
  bx_cpu. sregs[BX_SEG_REG_CS].selector.index =     0x0000;
  ...
  bx_cpu. sregs[BX_SEG_REG_CS].cache.u.segment.base         = 0x000F0000;
  ...
  bx_cpu. sregs[BX_SEG_REG_CS].cache.u.segment.d_b = 0; /* 16bit default size */
  bx_cpu. sregs[BX_SEG_REG_CS].cache.u.segment.avl = 0;
#endif


  /* SS (Stack Segment) and descriptor cache */
  bx_cpu. sregs[BX_SEG_REG_SS].selector.value =     0x0000;
#if BX_CPU_LEVEL >= 2
  bx_cpu. sregs[BX_SEG_REG_SS].selector.index =     0x0000;
  ...
  bx_cpu. sregs[BX_SEG_REG_SS].cache.u.segment.d_b = 0; /* 16bit default size */
  bx_cpu. sregs[BX_SEG_REG_SS].cache.u.segment.avl = 0;
#endif


  /* DS (Data Segment) and descriptor cache */
  bx_cpu. sregs[BX_SEG_REG_DS].selector.value =     0x0000;
#if BX_CPU_LEVEL >= 2
  bx_cpu. sregs[BX_SEG_REG_DS].selector.index =     0x0000;
  ...
  bx_cpu. sregs[BX_SEG_REG_DS].cache.u.segment.d_b = 0; /* 16bit default size */
  bx_cpu. sregs[BX_SEG_REG_DS].cache.u.segment.avl = 0;
#endif


  /* ES (Extra Segment) and descriptor cache */
  bx_cpu. sregs[BX_SEG_REG_ES].selector.value =     0x0000;
#if BX_CPU_LEVEL >= 2
  bx_cpu. sregs[BX_SEG_REG_ES].selector.index =     0x0000;
  ...
  bx_cpu. sregs[BX_SEG_REG_ES].cache.u.segment.d_b = 0; /* 16bit default size */
...
#if BX_CPU_LEVEL >= 3
  bx_cpu. sregs[BX_SEG_REG_FS].selector.value =     0x0000;
  bx_cpu. sregs[BX_SEG_REG_FS].selector.index =     0x0000;
  ...
  bx_cpu. sregs[BX_SEG_REG_FS].cache.u.segment.d_b = 0; /* 16bit default size */
...
#if BX_CPU_LEVEL >= 3
  bx_cpu. sregs[BX_SEG_REG_GS].selector.value =     0x0000;
  bx_cpu. sregs[BX_SEG_REG_GS].selector.index =     0x0000;
  ...
  bx_cpu. sregs[BX_SEG_REG_GS].cache.u.segment.d_b = 0; /* 16bit default size */
...
#endif
#if   BX_CPU_LEVEL == 3
  bx_cpu. dr6 = 0xFFFF1FF0;
  bx_cpu. dr7 = 0x00000400;
#elif BX_CPU_LEVEL == 4
  bx_cpu. dr6 = 0xFFFF1FF0;
  bx_cpu. dr7 = 0x00000400;
#elif BX_CPU_LEVEL == 5
  bx_cpu. dr6 = 0xFFFF0FF0;
  bx_cpu. dr7 = 0x00000400;
#elif BX_CPU_LEVEL == 6
  bx_cpu. dr6 = 0xFFFF0FF0;
  bx_cpu. dr7 = 0x00000400;
...
#  error "DR6,7: CPU > 6"
  ...
  bx_cpu. cr0.val32 |= 0x60000000;
#endif

  // handle reserved bits
#if BX_CPU_LEVEL == 3
  // reserved bits all set to 1 on 386
  bx_cpu. cr0.val32 |= 0x7ffffff0;
#elif BX_CPU_LEVEL >= 4
  // bit 4 is hardwired to 1 on all x86
  bx_cpu. cr0.val32 |= 0x00000010;
#endif
#endif // CPU >= 2
...
/* initialise MSR registers to defaults */
  ...
  bx_cpu. msr.apicbase = (APIC_BASE_ADDR << 12) + 0x900;
#endif
  ...
  bx_cpu. bytesleft = 0;
  bx_cpu. fetch_ptr = NULL;
  bx_cpu. prev_linear_page = 0;
  bx_cpu. prev_phy_page = 0;
  bx_cpu. max_phy_addr = 0;

#if BX_DEBUGGER
#ifdef MAGIC_BREAKPOINT
  bx_cpu. magic_break = 0;
#endif
  bx_cpu. stop_reason = STOP_NO_REASON;
  bx_cpu. trace = 0;
#endif
  ...
  if (BX_BOOTSTRAP_PROCESSOR == apic_id)
  {
    // boot normally
    bx_cpu. bsp = 1;
    bx_cpu. msr.apicbase |= 0x0100;	/* set bit 8 BSP */
    BX_INFO(("CPU[%d] is the bootstrap processor", apic_id));
  } else {
    // it's an application processor, halt until IPI is heard.
    bx_cpu. bsp = 0;
    bx_cpu. msr.apicbase &= ~0x0100;	/* clear bit 8 BSP */
