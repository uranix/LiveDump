...
//BX_DEBUG(( "TASKING: ENTER" ));
  ...
  if (bx_cpu. tr.cache.type <= 3) {
    // sanity check type: cannot have busy bit
    BX_ASSERT ((bx_cpu. tr.cache.type & 2) == 0);
    obase32 = bx_cpu. tr.cache.u.tss286.base;
    old_TSS_max   = 43;
    old_TSS_limit = bx_cpu. tr.cache.u.tss286.limit;
    }
  else {
    obase32 = bx_cpu. tr.cache.u.tss386.base;
    old_TSS_max   = 103;
    old_TSS_limit = bx_cpu. tr.cache.u.tss386.limit_scaled;
    }
...
if (ss_descriptor.u.segment.d_b && (tss_descriptor->type<9)) {
  ...
  if ( (source==BX_TASK_FROM_JUMP) || (source==BX_TASK_FROM_IRET) ) {
    // Bit is cleared
    access_linear(bx_cpu. gdtr.base +
                  bx_cpu. tr.selector.index*8 + 4,
                  4, 0, BX_READ, &temp32);
    temp32 &= ~0x00000200;
    access_linear(bx_cpu. gdtr.base +
                  bx_cpu. tr.selector.index*8 + 4,
  ...
  if (bx_cpu. tr.cache.type <= 3) {
    ...
    temp16 = bx_cpu. sregs[BX_SEG_REG_ES].selector.value;
                 access_linear(obase32 + 34, 2, 0, BX_WRITE, &temp16);
    temp16 = bx_cpu. sregs[BX_SEG_REG_CS].selector.value;
                 access_linear(obase32 + 36, 2, 0, BX_WRITE, &temp16);
    temp16 = bx_cpu. sregs[BX_SEG_REG_SS].selector.value;
                 access_linear(obase32 + 38, 2, 0, BX_WRITE, &temp16);
    temp16 = bx_cpu. sregs[BX_SEG_REG_DS].selector.value;
    ...
    temp16 = bx_cpu. sregs[BX_SEG_REG_FS].selector.value;
                  access_linear(obase32 + 0x58, 2, 0, BX_WRITE, &temp16);
    temp16 = bx_cpu. sregs[BX_SEG_REG_GS].selector.value;
...
if ( source==BX_TASK_FROM_CALL_OR_INT ) {
  ...
  if ( (source==BX_TASK_FROM_JUMP) || (source==BX_TASK_FROM_CALL_OR_INT) ) {
    ...
    access_linear(bx_cpu. gdtr.base + tss_selector->index*8 + 4,
                  4, 0, BX_READ, &dword2);
    dword2 |= 0x00000200;
    access_linear(bx_cpu. gdtr.base + tss_selector->index*8 + 4,
                  4, 0, BX_WRITE, &dword2);
  ...
  bx_cpu. sregs[BX_SEG_REG_ES].selector = es_selector;
  parse_selector(raw_cs_selector, &cs_selector);
  bx_cpu. sregs[BX_SEG_REG_CS].selector = cs_selector;
  parse_selector(raw_ss_selector, &ss_selector);
  bx_cpu. sregs[BX_SEG_REG_SS].selector = ss_selector;
  parse_selector(raw_ds_selector, &ds_selector);
  bx_cpu. sregs[BX_SEG_REG_DS].selector = ds_selector;
  parse_selector(raw_fs_selector, &fs_selector);
  bx_cpu. sregs[BX_SEG_REG_FS].selector = fs_selector;
  parse_selector(raw_gs_selector, &gs_selector);
  bx_cpu. sregs[BX_SEG_REG_GS].selector = gs_selector;
  parse_selector(raw_ldt_selector, &ldt_selector);
  bx_cpu. ldtr.selector                 = ldt_selector;
  ...
  bx_cpu. sregs[BX_SEG_REG_FS].cache.valid = 0;
  bx_cpu. sregs[BX_SEG_REG_GS].cache.valid = 0;
...
// ??? is LDT loaded in v8086 mode
  if ( (raw_ldt_selector & 0xfffc) != 0 ) {
    ...
    bx_cpu. ldtr.cache = ldt_descriptor;
    }
  else {
  ...
  if (v8086_mode()) {
    ...
    load_seg_reg(&bx_cpu. sregs[BX_SEG_REG_CS], raw_cs_selector);
    load_seg_reg(&bx_cpu. sregs[BX_SEG_REG_SS], raw_ss_selector);
    load_seg_reg(&bx_cpu. sregs[BX_SEG_REG_DS], raw_ds_selector);
    load_seg_reg(&bx_cpu. sregs[BX_SEG_REG_ES], raw_es_selector);
    load_seg_reg(&bx_cpu. sregs[BX_SEG_REG_FS], raw_fs_selector);
    load_seg_reg(&bx_cpu. sregs[BX_SEG_REG_GS], raw_gs_selector);
    }
  else {
  ...
  if ( (raw_cs_selector & 0xfffc) != 0 ) {
    ...
    bx_cpu. sregs[BX_SEG_REG_CS].cache    = cs_descriptor;
    }
  else {
...
#endif
    // All checks pass, fill in shadow cache
    bx_cpu. sregs[BX_SEG_REG_SS].cache    = ss_descriptor;
    }
  else {
  ...
  if ( (raw_ds_selector & 0xfffc) != 0 ) {
    ...
    // All checks pass, fill in shadow cache
    bx_cpu. sregs[BX_SEG_REG_DS].cache    = ds_descriptor;
    }
  else {
  ...
  if ( (raw_es_selector & 0xfffc) != 0 ) {
    ...
    // All checks pass, fill in shadow cache
    bx_cpu. sregs[BX_SEG_REG_ES].cache    = es_descriptor;
    }
  else {
  ...
  if ( (raw_fs_selector & 0xfffc) != 0 ) { // not NULL
    ...
    // All checks pass, fill in shadow cache
    bx_cpu. sregs[BX_SEG_REG_FS].cache    = fs_descriptor;
    }
  else {
  ...
  if ( (raw_gs_selector & 0xfffc) != 0 ) {
    ...
    // All checks pass, fill in shadow cache
    bx_cpu. sregs[BX_SEG_REG_GS].cache    = gs_descriptor;
    }
  else {
  ...
  if ((tss_descriptor->type>=9) && (trap_word & 0x0001)) {
    bx_cpu. debug_trap |= 0x00008000; // BT flag in DR6
...
bx_cpu_c::get_SS_ESP_from_TSS(unsigned pl, Bit16u *ss, Bit32u *esp)
{
  ...
  if (bx_cpu. tr.cache.type==9) {
    ...
    TSSstackaddr = 8*pl + 4;
    if ( (TSSstackaddr+7) >
         bx_cpu. tr.cache.u.tss386.limit_scaled )
      exception(BX_TS_EXCEPTION,
                bx_cpu. tr.selector.value & 0xfffc, 0);

    access_linear(bx_cpu. tr.cache.u.tss386.base +
      TSSstackaddr+4, 2, 0, BX_READ, ss);
    access_linear(bx_cpu. tr.cache.u.tss386.base +
      TSSstackaddr,   4, 0, BX_READ, esp);
    }
  else if (bx_cpu. tr.cache.type==1) {
    // 16-bit TSS
    Bit16u temp16;
    Bit32u TSSstackaddr;

    TSSstackaddr = 4*pl + 2;
    if ( (TSSstackaddr+4) > bx_cpu. tr.cache.u.tss286.limit )
      exception(BX_TS_EXCEPTION,
                bx_cpu. tr.selector.value & 0xfffc, 0);

    access_linear(bx_cpu. tr.cache.u.tss286.base +
