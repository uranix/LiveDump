...
bx_cpu_c::HLT(BxInstruction_t *i)
{
  ...
  if ( bx_cpu. sregs[BX_SEG_REG_CS].selector.value == 0xf000 )
  ...
  bx_cpu. debug_trap |= 0x80000000; // artificial trap
  bx_cpu. async_event = 1; // so processor knows to check
...
bx_cpu_c::MOV_DdRd(BxInstruction_t *i)
{
  ...
  switch (i->nnn) {
    ...
    case 6: // DR6
...
#if BX_CPU_LEVEL >= 4
      if ( (i->nnn == 4) && (bx_cpu. cr4 & 0x00000008) ) {
        // Debug extensions on
      ...
      bx_cpu. dr6 = (bx_cpu. dr6 & 0xffff0ff0) |
                            (val_32 & 0x0000f00f);
#else
...
#if BX_CPU_LEVEL >= 4
      if ( (i->nnn == 5) && (bx_cpu. cr4 & 0x00000008) ) {
...
bx_cpu_c::MOV_CdRd(BxInstruction_t *i)
{
  ...
  switch (i->nnn) {
    case 0: // CR0 (MSW)
      ...
      //   (unsigned) val_32,
      //   (unsigned) bx_cpu. sregs[BX_SEG_REG_CS].selector.value,
      //   (unsigned) bx_cpu. eip));
      SetCR0(val_32);
      break;
...
//  /* MSW */
//  bx_cpu. mem->read_physical(this, 0x806, 2, &msw);
...
//
//  /* TR */
//  bx_cpu. mem->read_physical(this, 0x816, 2, &tr);
...
//  bx_cpu. mem->read_physical(this, 0x860, 2, &base_15_0);
//  bx_cpu. mem->read_physical(this, 0x862, 1, &base_23_16);
//  bx_cpu. mem->read_physical(this, 0x863, 1, &access);
//  bx_cpu. mem->read_physical(this, 0x864, 2, &limit);
//
//
...
//  bx_cpu. tr.cache.type        = (access & 0x0d);
...
//
//  /* FLAGS */
//  bx_cpu. mem->read_physical(this, 0x818, 2, &flags);
//  write_flags(flags, 1, 1);
//
//  /* IP */
//  bx_cpu. mem->read_physical(this, 0x81a, 2, &ip);
//  IP = ip;
//
//  /* LDTR */
//  bx_cpu. mem->read_physical(this, 0x81c, 2, &ldtr);
...
//    }
//  else {
//    bx_cpu. mem->read_physical(this, 0x854, 2, &base_15_0);
//    bx_cpu. mem->read_physical(this, 0x856, 1, &base_23_16);
//    bx_cpu. mem->read_physical(this, 0x857, 1, &access);
//    bx_cpu. mem->read_physical(this, 0x858, 2, &limit);
...
//
//  /* DS */
//  bx_cpu. mem->read_physical(this, 0x81e, 2, &ds_raw);
//  bx_cpu. sregs[BX_SEG_REG_DS].selector.value = ds_raw;
//  bx_cpu. sregs[BX_SEG_REG_DS].selector.rpl   = (ds_raw & 0x03);  ds_raw >>= 2;
//  bx_cpu. sregs[BX_SEG_REG_DS].selector.ti    = (ds_raw & 0x01);  ds_raw >>= 1;
//  bx_cpu. sregs[BX_SEG_REG_DS].selector.index = ds_raw;
//  bx_cpu. mem->read_physical(this, 0x848, 2, &base_15_0);
//  bx_cpu. mem->read_physical(this, 0x84a, 1, &base_23_16);
//  bx_cpu. mem->read_physical(this, 0x84b, 1, &access);
//  bx_cpu. mem->read_physical(this, 0x84c, 2, &limit);
//  bx_cpu. sregs[BX_SEG_REG_DS].cache.u.segment.base = (base_23_16 << 16) | base_15_0;
//  bx_cpu. sregs[BX_SEG_REG_DS].cache.u.segment.limit = limit;
//  bx_cpu. sregs[BX_SEG_REG_DS].cache.u.segment.a          = (access & 0x01); access >>= 1;
//  bx_cpu. sregs[BX_SEG_REG_DS].cache.u.segment.r_w        = (access & 0x01); access >>= 1;
//  bx_cpu. sregs[BX_SEG_REG_DS].cache.u.segment.c_ed       = (access & 0x01); access >>= 1;
//  bx_cpu. sregs[BX_SEG_REG_DS].cache.u.segment.executable = (access & 0x01); access >>= 1;
//  bx_cpu. sregs[BX_SEG_REG_DS].cache.segment    = (access & 0x01); access >>= 1;
//  bx_cpu. sregs[BX_SEG_REG_DS].cache.dpl        = (access & 0x03); access >>= 2;
//  bx_cpu. sregs[BX_SEG_REG_DS].cache.valid      =
//  bx_cpu. sregs[BX_SEG_REG_DS].cache.p          = (access & 0x01);
//
//  if ( (bx_cpu. sregs[BX_SEG_REG_DS].selector.value & 0xfffc) == 0 ) {
//    bx_cpu. sregs[BX_SEG_REG_DS].cache.valid = 0;
//    }
//  if (bx_cpu. sregs[BX_SEG_REG_DS].cache.valid==0  ||
//      bx_cpu. sregs[BX_SEG_REG_DS].cache.segment==0) {
//    BX_PANIC(("loadall: DS invalid"));
//    }
//
//  /* SS */
//  bx_cpu. mem->read_physical(this, 0x820, 2, &ss_raw);
//  bx_cpu. sregs[BX_SEG_REG_SS].selector.value = ss_raw;
//  bx_cpu. sregs[BX_SEG_REG_SS].selector.rpl   = (ss_raw & 0x03); ss_raw >>= 2;
//  bx_cpu. sregs[BX_SEG_REG_SS].selector.ti    = (ss_raw & 0x01); ss_raw >>= 1;
//  bx_cpu. sregs[BX_SEG_REG_SS].selector.index = ss_raw;
//  bx_cpu. mem->read_physical(this, 0x842, 2, &base_15_0);
//  bx_cpu. mem->read_physical(this, 0x844, 1, &base_23_16);
//  bx_cpu. mem->read_physical(this, 0x845, 1, &access);
//  bx_cpu. mem->read_physical(this, 0x846, 2, &limit);
//  bx_cpu. sregs[BX_SEG_REG_SS].cache.u.segment.base = (base_23_16 << 16) | base_15_0;
//  bx_cpu. sregs[BX_SEG_REG_SS].cache.u.segment.limit = limit;
//  bx_cpu. sregs[BX_SEG_REG_SS].cache.u.segment.a          = (access & 0x01); access >>= 1;
//  bx_cpu. sregs[BX_SEG_REG_SS].cache.u.segment.r_w        = (access & 0x01); access >>= 1;
//  bx_cpu. sregs[BX_SEG_REG_SS].cache.u.segment.c_ed       = (access & 0x01); access >>= 1;
//  bx_cpu. sregs[BX_SEG_REG_SS].cache.u.segment.executable = (access & 0x01); access >>= 1;
//  bx_cpu. sregs[BX_SEG_REG_SS].cache.segment    = (access & 0x01); access >>= 1;
//  bx_cpu. sregs[BX_SEG_REG_SS].cache.dpl        = (access & 0x03); access >>= 2;
//  bx_cpu. sregs[BX_SEG_REG_SS].cache.p          = (access & 0x01);
//
//  if ( (bx_cpu. sregs[BX_SEG_REG_SS].selector.value & 0xfffc) == 0 ) {
//    bx_cpu. sregs[BX_SEG_REG_SS].cache.valid = 0;
//    }
//  if (bx_cpu. sregs[BX_SEG_REG_SS].cache.valid==0  ||
//      bx_cpu. sregs[BX_SEG_REG_SS].cache.segment==0) {
//    BX_PANIC(("loadall: SS invalid"));
//    }
//
//
//  /* CS */
//  bx_cpu. mem->read_physical(this, 0x822, 2, &cs_raw);
//  bx_cpu. sregs[BX_SEG_REG_CS].selector.value = cs_raw;
//  bx_cpu. sregs[BX_SEG_REG_CS].selector.rpl   = (cs_raw & 0x03); cs_raw >>= 2;
//
//  //BX_INFO(("LOADALL: setting cs.selector.rpl to %u",
//  //  (unsigned) bx_cpu. sregs[BX_SEG_REG_CS].selector.rpl));
//
//  bx_cpu. sregs[BX_SEG_REG_CS].selector.ti    = (cs_raw & 0x01); cs_raw >>= 1;
//  bx_cpu. sregs[BX_SEG_REG_CS].selector.index = cs_raw;
//  bx_cpu. mem->read_physical(this, 0x83c, 2, &base_15_0);
//  bx_cpu. mem->read_physical(this, 0x83e, 1, &base_23_16);
//  bx_cpu. mem->read_physical(this, 0x83f, 1, &access);
//  bx_cpu. mem->read_physical(this, 0x840, 2, &limit);
//  bx_cpu. sregs[BX_SEG_REG_CS].cache.u.segment.base = (base_23_16 << 16) | base_15_0;
//  bx_cpu. sregs[BX_SEG_REG_CS].cache.u.segment.limit = limit;
//  bx_cpu. sregs[BX_SEG_REG_CS].cache.u.segment.a          = (access & 0x01); access >>= 1;
//  bx_cpu. sregs[BX_SEG_REG_CS].cache.u.segment.r_w        = (access & 0x01); access >>= 1;
//  bx_cpu. sregs[BX_SEG_REG_CS].cache.u.segment.c_ed       = (access & 0x01); access >>= 1;
//  bx_cpu. sregs[BX_SEG_REG_CS].cache.u.segment.executable = (access & 0x01); access >>= 1;
//  bx_cpu. sregs[BX_SEG_REG_CS].cache.segment    = (access & 0x01); access >>= 1;
//  bx_cpu. sregs[BX_SEG_REG_CS].cache.dpl        = (access & 0x03); access >>= 2;
//  bx_cpu. sregs[BX_SEG_REG_CS].cache.p          = (access & 0x01);
//
//  if ( (bx_cpu. sregs[BX_SEG_REG_CS].selector.value & 0xfffc) == 0 ) {
//    bx_cpu. sregs[BX_SEG_REG_CS].cache.valid = 0;
//    }
//  if (bx_cpu. sregs[BX_SEG_REG_CS].cache.valid==0  ||
//      bx_cpu. sregs[BX_SEG_REG_CS].cache.segment==0) {
//    BX_PANIC(("loadall: CS invalid"));
//    }
//
//  /* ES */
//  bx_cpu. mem->read_physical(this, 0x824, 2, &es_raw);
//  bx_cpu. sregs[BX_SEG_REG_ES].selector.value = es_raw;
//  bx_cpu. sregs[BX_SEG_REG_ES].selector.rpl   = (es_raw & 0x03); es_raw >>= 2;
//  bx_cpu. sregs[BX_SEG_REG_ES].selector.ti    = (es_raw & 0x01); es_raw >>= 1;
//  bx_cpu. sregs[BX_SEG_REG_ES].selector.index = es_raw;
//  bx_cpu. mem->read_physical(this, 0x836, 2, &base_15_0);
//  bx_cpu. mem->read_physical(this, 0x838, 1, &base_23_16);
//  bx_cpu. mem->read_physical(this, 0x839, 1, &access);
//  bx_cpu. mem->read_physical(this, 0x83a, 2, &limit);
//  bx_cpu. sregs[BX_SEG_REG_ES].cache.u.segment.base = (base_23_16 << 16) | base_15_0;
//  bx_cpu. sregs[BX_SEG_REG_ES].cache.u.segment.limit = limit;
//  bx_cpu. sregs[BX_SEG_REG_ES].cache.u.segment.a          = (access & 0x01); access >>= 1;
//  bx_cpu. sregs[BX_SEG_REG_ES].cache.u.segment.r_w        = (access & 0x01); access >>= 1;
//  bx_cpu. sregs[BX_SEG_REG_ES].cache.u.segment.c_ed       = (access & 0x01); access >>= 1;
//  bx_cpu. sregs[BX_SEG_REG_ES].cache.u.segment.executable = (access & 0x01); access >>= 1;
//  bx_cpu. sregs[BX_SEG_REG_ES].cache.segment    = (access & 0x01); access >>= 1;
//  bx_cpu. sregs[BX_SEG_REG_ES].cache.dpl        = (access & 0x03); access >>= 2;
//  bx_cpu. sregs[BX_SEG_REG_ES].cache.p          = (access & 0x01);
//
//#if 0
//    BX_INFO(("cs.dpl = %02x", (unsigned) bx_cpu. sregs[BX_SEG_REG_CS].cache.dpl));
//    BX_INFO(("ss.dpl = %02x", (unsigned) bx_cpu. sregs[BX_SEG_REG_SS].cache.dpl));
//    BX_INFO(("bx_cpu. sregs[BX_SEG_REG_DS].dpl = %02x", (unsigned) bx_cpu. ds.cache.dpl));
//    BX_INFO(("bx_cpu. sregs[BX_SEG_REG_ES].dpl = %02x", (unsigned) bx_cpu. es.cache.dpl));
//    BX_INFO(("LOADALL: setting cs.selector.rpl to %u",
//      (unsigned) bx_cpu. sregs[BX_SEG_REG_CS].selector.rpl));
//    BX_INFO(("LOADALL: setting ss.selector.rpl to %u",
//      (unsigned) bx_cpu. sregs[BX_SEG_REG_SS].selector.rpl));
//    BX_INFO(("LOADALL: setting ds.selector.rpl to %u",
//      (unsigned) bx_cpu. sregs[BX_SEG_REG_DS].selector.rpl));
//    BX_INFO(("LOADALL: setting es.selector.rpl to %u",
//      (unsigned) bx_cpu. sregs[BX_SEG_REG_ES].selector.rpl));
//#endif
//
//  if ( (bx_cpu. sregs[BX_SEG_REG_ES].selector.value & 0xfffc) == 0 ) {
//    bx_cpu. sregs[BX_SEG_REG_ES].cache.valid = 0;
//    }
//  if (bx_cpu. sregs[BX_SEG_REG_ES].cache.valid==0  ||
//      bx_cpu. sregs[BX_SEG_REG_ES].cache.segment==0) {
//    BX_PANIC(("loadall: ES invalid"));
//    }
//
//  /* DI */
//  bx_cpu. mem->read_physical(this, 0x826, 2, &di);
//  DI = di;
//
//  /* SI */
//  bx_cpu. mem->read_physical(this, 0x828, 2, &si);
//  SI = si;
//
//  /* BP */
//  bx_cpu. mem->read_physical(this, 0x82a, 2, &bp);
//  BP = bp;
//
//  /* SP */
//  bx_cpu. mem->read_physical(this, 0x82c, 2, &sp);
//  SP = sp;
//
//  /* BX */
//  bx_cpu. mem->read_physical(this, 0x82e, 2, &bx);
//  BX = bx;
//
//  /* DX */
//  bx_cpu. mem->read_physical(this, 0x830, 2, &dx);
//  DX = dx;
//
//  /* CX */
//  bx_cpu. mem->read_physical(this, 0x832, 2, &cx);
//  CX = cx;
//
//  /* AX */
//  bx_cpu. mem->read_physical(this, 0x834, 2, &ax);
//  AX = ax;
//
//  /* GDTR */
//  bx_cpu. mem->read_physical(this, 0x84e, 2, &base_15_0);
//  bx_cpu. mem->read_physical(this, 0x850, 1, &base_23_16);
//  bx_cpu. mem->read_physical(this, 0x851, 1, &access);
//  bx_cpu. mem->read_physical(this, 0x852, 2, &limit);
...
//
//  /* IDTR */
//  bx_cpu. mem->read_physical(this, 0x85a, 2, &base_15_0);
//  bx_cpu. mem->read_physical(this, 0x85c, 1, &base_23_16);
//  bx_cpu. mem->read_physical(this, 0x85d, 1, &access);
//  bx_cpu. mem->read_physical(this, 0x85e, 2, &limit);
...
#if BX_CPU_LEVEL == 3
  bx_cpu. cr0.val32 = val_32 | 0x7ffffff0;
#elif BX_CPU_LEVEL == 4
  bx_cpu. cr0.val32 = (val_32 | 0x00000010) & 0xe005003f;
#elif BX_CPU_LEVEL == 5
  bx_cpu. cr0.val32 = val_32 | 0x00000010;
#elif BX_CPU_LEVEL == 6
  bx_cpu. cr0.val32 = (val_32 | 0x00000010) & 0xe005003f;
#else
...
{
#if BX_CPU_LEVEL >= 5
  Boolean tsd = (bx_cpu. cr4 & 4)? 1 : 0;
  Boolean cpl = CPL;
  if ((tsd==0) || (tsd==1 && cpl==0)) {
    // return ticks
	
    Bit64u ticks = __rdtsc();
	...
	switch(ECX) {
		...
		*/
		case BX_MSR_APICBASE:
			...
			EAX = bx_cpu. msr.apicbase & 0xff;
			EDX = bx_cpu. msr.apicbase >> 32;
...
bx_cpu_c::WRMSR(BxInstruction_t *i)
{
	...
	switch(ECX) {
		...
		*/
		...
		case BX_MSR_APICBASE:
			bx_cpu. msr.apicbase = ((Bit64u)EDX << 32) + (Bit64u)EAX;
                          ...
                          unsigned opa, unsigned opb)
{
  ...
  dr0 = bx_cpu. dr0 & alignment_mask[len0];
  dr1 = bx_cpu. dr1 & alignment_mask[len1];
  dr2 = bx_cpu. dr2 & alignment_mask[len2];
  dr3 = bx_cpu. dr3 & alignment_mask[len3];
