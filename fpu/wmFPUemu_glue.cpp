...
access_limit = 0xff;
  ...
  entry_sel_off.offset = bx_cpu. prev_eip;
  entry_sel_off.selector = bx_cpu. sregs[BX_SEG_REG_CS].selector.value;

// should set these fields to 0 if mem operand not used
  data_address = (void *) i->rm_addr;
  data_sel_off.offset = i->rm_addr;
  data_sel_off.selector = bx_cpu. sregs[i->seg].selector.value;
